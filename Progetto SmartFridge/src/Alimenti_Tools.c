 ...
 * OCCORRENZA														*/
int getAlimento(alimento alimenti[],int Lunghezza_Alimenti,char Parametri_Ricerca[]){

	int i;
	char StringTempParametri[LUNGHEZZA_STRINGA];
	removeFirstLastSpace(Parametri_Ricerca,StringTempParametri,LUNGHEZZA_STRINGA);
	...
	for(i=0;i<Lunghezza_Alimenti;i++){
		...
		removeFirstLastSpace(alimenti[i].Nome,StringTempVettore,LUNGHEZZA_STRINGA);
		toLowerString(StringTempVettore,StringTempVettore);

		//se le due stringhe sono uguali
		if(strcmp(StringTempParametri,StringTempVettore) == 0) return i;
	}
...
int Visualizza_Alimenti(alimento alimenti[],int Lunghezza_Vettore){
	...
	for (i=0;i<Lunghezza_Vettore-1;i++){
		...
		printf("%d - %s \t| Peso: %d \t| Id: %d   |   Quantita: %d\n", i, alimenti[i].Nome,
				alimenti[i].Peso , alimenti[i].ID_Alimento,quantita);
	}
	return 1;
}
